"""Warning Workflow Processor.

Triggered when a sensor transitions into the 'Warning' state.
...
from clients.smtp_client import send_warning_notification
...
WARNING_STATE_NAME = "Warning"
DEFAULT_ALERT_MESSAGE = "Sensor reported a WARNING condition"


class WarningWorkflow:
    """Workflow handler for WARNING sensor events."""

    async def execute(self, payload: Any) -> None:
        logger.info("Executing WarningWorkflow...")
        ...
        logger.info("Processing WarningWorkflow for sensor_id: %s", sensor_id)

        raw_status = extract_field(payload, "status", "Warning")
        status_str = sanitize_status(raw_status, default="Warning")
        ...
        with session_scope(SessionLocal) as session:
            ...
            try:
                warning_state = alert_state_repo.get_by_name(WARNING_STATE_NAME)
                if warning_state is None:
                    ...
                    alert = alert_repo.create(
                        sensor_id=sensor_id,
                        state_id=warning_state.state_id,
        ...
        # ------------------------------------------------------------------
        # Phase 2: Send warning email to support team (outside DB transaction)
        # ------------------------------------------------------------------
        ...
        email_sent = False
        try:
            email_sent = await asyncio.to_thread(send_warning_notification, notification_payload)
        except Exception:
            logger.exception("Unexpected error dispatching warning notification for sensor %s", sensor_id)
            email_sent = False
        ...
        with session_scope(SessionLocal) as session:
            ...
            if alert_id is not None:
                try:
                    escalation_repo.create(
                        ...
                        cc_emails=None,
                        email_subject="Sensor Warning Alert",
                        email_body=message,
            ...
            try:
                log_entry = log_repo.create(
                    ...
                    log_message=f"Sensor registered WARNING. Support notification {'sent' if email_sent else 'failed'}.",
        ...
        logger.info(
            "Warning workflow completed for sensor %s (alert_id=%s, email_sent=%s)",
            sensor_id,
            alert_id,
            email_sent,
        )
...
def process(payload: Any) -> None:
    ...
    logger.info("Received request to process Warning workflow payload.")
