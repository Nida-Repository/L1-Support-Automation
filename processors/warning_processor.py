...
#  ALERT_STATES row that represents a sensor-warning condition.
...
class WarningWorkflow:
    ...
    async def execute(self, payload):
        ...
        with session_scope(SessionLocal) as session:
            ...
            raw_status = _extract_field(payload, "status", "Warning")
            # --- FIX: Sanitize status to prevent 'SensorStatus.WARNING' leak ---
            ...
            # ----------------------------------------------------------------
            #  Create ALERT_HISTORY record with the Warning state
            # ----------------------------------------------------------------
            alert_id = None
            try:
            ...
            # ----------------------------------------------------------------
            #  Send the warning email to the support team
            # ----------------------------------------------------------------
            notification_payload = {
                "site_name": site_name,
                "sensor_name": sensor_name,
                "status": status_str,  # Clean string passed here
                "message": message,
            ...
            try:
                email_sent = await asyncio.to_thread(
                    send_warning_notification, notification_payload
                )
            except Exception:
                logger.exception(
                    "Unexpected error while sending warning notification for sensor %s",
                    sensor_id,
                )
                email_sent = False
            ...
            # ----------------------------------------------------------------
            #  Create SENSOR_LOGS entry documenting the warning
            # ----------------------------------------------------------------
            try:
                log_entry = log_repo.create(
                    ...
                    log_message=(
                        f"Sensor registered WARNING. Support notification "
